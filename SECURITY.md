...
This is the on-device BT Bridge agent app. In-scope concerns include: handling of templates pushed
from the broker (path traversal in on-device storage, malformed/truncated BLE frames causing crashes),
parsing of untrusted BLE peripheral data, and any way a malicious broker or template could compromise
the device beyond the app's sandbox. The agent connects to a user-specified broker over the LAN; the
broker is unauthenticated by design (see the broker repo's security policy).
